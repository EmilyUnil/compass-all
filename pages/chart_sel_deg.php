...
__halt_compiler();
$garnizon = filter_input(INPUT_GET, 'garnizon', FILTER_SANITIZE_STRING, ['options' => ['default' => $_SESSION['user_garnizon']]]);
$startDate = filter_input(INPUT_GET, 'start', FILTER_SANITIZE_STRING);
$endDate = filter_input(INPUT_GET, 'end', FILTER_SANITIZE_STRING);
$numberNaim = filter_input(INPUT_GET, 'number_naim', FILTER_SANITIZE_STRING);
$today = new DateTime();
$yesterday = (clone $today)->sub(new DateInterval('P1D'))->format('d.m.Y');
$oneMonthAgo = (clone $today)->sub(new DateInterval('P30D'))->format('d.m.Y');
...
$initialGarnisonIndex = $garnizon !== null && array_key_exists($garnizon, $garnizonNames) ? $garnizon : $_SESSION['user_garnizon'];
$initialGarnisonText = isset($garnizonNames[$initialGarnisonIndex]) ? strtoupper($garnizonNames[$initialGarnisonIndex]) : 'ГАРНИЗОН';
$conn->close();
?>
<!DOCTYPE html>
<html lang="ru">
...
<body>
    ...
    <div class="container">
        <div class="date-selector">
            ...
            <div class="dropdown">
                <button class="btn btn-primary <?php if ($_SESSION['dostup'][ACCESS_SVODKA] != '1' && $_SESSION['dostup'][ACCESS_SVODKA] != '3' && $_SESSION['dostup'][ACCESS_SVODKA] != '5') echo 'dropdown-toggle'; ?>"
                        type="button"
                        id="garnizon-dropdown"
                        <?php if ($_SESSION['dostup'][ACCESS_SVODKA] == '1' || $_SESSION['dostup'][ACCESS_SVODKA] == '3' || $_SESSION['dostup'][ACCESS_SVODKA] == '5') echo 'disabled'; ?>
                        <?php if ($_SESSION['dostup'][ACCESS_SVODKA] != '1' && $_SESSION['dostup'][ACCESS_SVODKA] != '3' && $_SESSION['dostup'][ACCESS_SVODKA] != '5') echo 'data-bs-toggle="dropdown"'; ?>>
                    <?php echo htmlspecialchars($initialGarnisonText); ?>
                </button>
                <?php if ($_SESSION['dostup'][ACCESS_SVODKA] != '1' && $_SESSION['dostup'][ACCESS_SVODKA] != '3' && $_SESSION['dostup'][ACCESS_SVODKA] != '5') { ?>
                    <ul class="dropdown-menu">
                        <?php if ($_SESSION['dostup'][ACCESS_SVODKA] == '2' || $_SESSION['dostup'][ACCESS_SVODKA] == '4') { ?>
...
async function loadAllIndicators() {
    try {
        const response = await $.ajax({ url: '../sql/dropdown_sel_deg.php' });
